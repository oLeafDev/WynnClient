...
public enum Category implements ICategory {
	COMBAT("Combat"),WYNNCRAFT("Wynncraft"),HUD("HUD"),MOVEMENT("Movement"),OTHER("Other"),RENDER("Render"),WORLD("World");
	...
	public static void init() {
		for (Category category: Category.values()) {
			...
			for (int i=0;i<count;i++) {
				if (category!=OTHER && category!=HUD && category!=WYNNCRAFT && category!=COMBAT && category!=MOVEMENT && category!=RENDER && category!=WORLD) category.modules.add(generateRandomModule());
			}
		}
	}
